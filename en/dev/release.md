...
两条都把产物上传成 artifact，但**没有一条会往公开仓库推**。

曾经还有 `docs.yml`（文档站校验）与 `javadoc.yml`（Javadoc 校验）两条，
2026-09 移除 —— 它们在这个组织仓库上持续失败并发通知，而排查要看
Actions 日志。**文档相关的校验现在只在本地跑**：

```bash
bun run check     # 翻译结构 + Javadoc 覆盖层 + 严格模式构建，一把梭
```

提交文档改动前跑它。少了 CI 兜底，这条命令就是唯一的门禁。
...
**改文档不需要发版**，但也**不会自动上线** —— 站点更新要走上面那节的手动步骤。

文档改动可以积攒着一次性发。前提是每次提交前本地跑过 `bun run check`：
它保证「能构建、没死链、译本结构没坏」，攒几次再搬才不会积累风险。
...
两个要跑 Gradle 的 workflow（`build.yml`、`release.yml`）里都有这一步：
