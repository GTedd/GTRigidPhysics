...
| 仓库 | 谁能进 | 是什么 |
|---|---|---|
| [`GTedd/GTRigidPhysics`](https://github.com/GTedd/GTRigidPhysics) | 所有人 | 公开的文档仓库。站上每页的铅笔指向它 |
| 插件源码仓库 | 团队成员 | `docs/` 的真相源，也是 Javadoc 译文覆盖层所在 |

**文档站的翻译提到公开那个。** 它就是铅笔按钮落地的地方，不需要任何权限。

**Javadoc 的翻译只能在源码仓库做** —— 覆盖层要跟 `.java` 原文逐字比对，
公开仓库里没有那些源文件，校验跑不起来。

!!! warning "给维护者：合并之后要立刻回流"

    公开仓库的 `docs/` 是源码仓库那份的**单向覆盖**副本。
    在公开仓库合掉的 PR，必须尽快把同样的改动带回源码仓库，
    否则下一次同步会把它抹掉。合了就搬，别攒着。
...
翻译不需要跑插件的测试套件，但**提交前请跑一次** `bun run check`
（翻译结构 + Javadoc 覆盖层 + 严格模式构建）—— 文档这条链路没有 CI 兜底。
